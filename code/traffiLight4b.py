...
while noInterupt == True:
		...
		for idx in range(0,timeRed):
			...
			client.publish("traffiLight/actualState/", "Red")
			client.publish("traffiLight/nextState/", "Red-Yellow")
			client.publish("traffiLight/timeTillNextState", ttns)
			client.publish("traffiLight/durationofNextState/", timeRedYellow)
			print("AS=Red")
			print("NS=Red-Yellow")
			print(ttns)
			print(timeRedYellow)
